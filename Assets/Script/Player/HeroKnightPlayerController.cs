...
public class HeroKnightPlayerController : MonoBehaviour
{
    ...
    [Tooltip("Matches HeroKnight's noBlood option - passed to the animator before the Death trigger.")]
    [SerializeField] private bool noBlood = false;
    [Tooltip("Optional - auto-found via GetComponent if left empty. Used so movement freezes while attacking/blocking.")]
    ...
    // Parameter names below intentionally mirror HeroKnight.cs so the same
    // Animator Controller layout/blend trees can be reused:
    //   AnimState (int: 0 idle / 1 run), Grounded (bool), AirSpeedY (float),
    //   Jump (trigger), Attack1/2/3 (triggers, in PlayerAttack.cs),
    //   Block (trigger, in PlayerAttack.cs), Hurt (trigger, in
    //   PlayerHealth.cs), Death (trigger), noBlood (bool).
    private static readonly int AnimStateParam = Animator.StringToHash("AnimState");
    private static readonly int GroundedParam = Animator.StringToHash("Grounded");
    private static readonly int AirSpeedYParam = Animator.StringToHash("AirSpeedY");
    ...
    public int FacingDirection => moveRight ? 1 : -1;

    /// <summary>
    /// True while something external (e.g. a flying enemy's lift/grab
    /// attack) has taken over the player's position - normal input,
    /// movement and physics are suspended until SetExternallyControlled(false)
    /// is called. See SetExternallyControlled below.
    /// </summary>
    public bool IsExternallyControlled { get; private set; }
    private RigidbodyType2D originalBodyType;

    /// <summary>
    /// True while the player is attacking or blocking - movement is frozen
    /// during this window so combat doesn't slide the player around.
    /// </summary>
    private bool IsActionLocked => playerAttack != null && (playerAttack.IsAttacking || playerAttack.IsBlocking);
    ...
    private void Update()
    {
        ...
        {
            HandleExternallyControlledState();
            return;
        }
    ...
    private void FixedUpdate()
    {
        ...
        if (IsExternallyControlled)
        {
            // Rigidbody2D is kinematic during this - whoever called
            // SetExternallyControlled(true) (e.g. a flying enemy's lift
            // attack) is responsible for moving the player via transform.
            return;
        }
    ...
    private void HandleExternallyControlledState()
    {
        currentMoveInput = 0f;
        jumpRequested = false;
        StopRunningSound();
    ...
    /// <summary>
    /// Hands control of the player over to (or back from) an external
    /// system - used for grab/lift attacks. While true: input, normal
    /// movement/physics and gravity are all suspended (Rigidbody2D becomes
    /// kinematic) so the caller can freely move the player's transform
    /// (e.g. following a flying enemy's lift point) without fighting normal
    /// player physics. Always call with false again to hand control back.
    /// </summary>
    ...
    /// <summary>
    /// Called instead of normal movement while attacking or blocking. Cancels
    /// any pending jump and zeroes horizontal velocity, but leaves vertical
    /// velocity alone so gravity still applies if this happens mid-air.
    /// </summary>
    private void HandleActionLockedMovement()
    {
        jumpRequested = false;
        wallSlideHoldTimer = 0f;
        rb.linearVelocity = new Vector2(0f, rb.linearVelocity.y);
    ...
    {
        // SetTrigger (not Play-by-name) so this works no matter what your
        // jump state is actually called in the Animator Controller - it just
        // needs a "Jump" trigger parameter wired to a transition.
    ...
    {
        bool grounded = IsGrounded && jumpGroundLockTimer <= 0f;
        bool isRunning = !IsActionLocked && Mathf.Abs(currentMoveInput) > 0.01f;

        anim.SetInteger(AnimStateParam, isRunning ? 1 : 0);
        anim.SetBool(GroundedParam, grounded);

        // AirSpeedY lets the Animator Controller pick between jump/fall (and
        // anything in between) purely from vertical velocity, same as
        // HeroKnight - no separate "fall" trigger needed.
        anim.SetFloat(AirSpeedYParam, rb.linearVelocity.y);
    }
    ...
    {
        bool shouldPlayRunSound = !IsActionLocked && IsGrounded && Mathf.Abs(currentMoveInput) > 0.01f;
    ...
    /// <summary>
    /// Kills the player: plays the Death animation/sound, spawns blood,
    /// shows the death UI and reloads the scene after a delay. Public so
    /// PlayerHealth (or anything else, e.g. traps/enemies) can call it
    /// directly instead of duplicating death handling elsewhere.
    /// </summary>
    public void Die()
    {
        if (IsDead)
        ...
        IsDead = true;
        PlayDeathSound();
        anim.SetBool(NoBloodParam, noBlood);
    ...
    /// <summary>
    /// Plays the Hurt animation. Called by PlayerHealth whenever the player
    /// takes damage but survives it.
    /// </summary>
    public void PlayHurt()
    {
        ...
        anim.SetTrigger(Animator.StringToHash("Hurt"));
    }
